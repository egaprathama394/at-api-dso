...
import rateOrder from '../endpoints/rateOrder';
import * as bodyRateOrder from '../data/rateOrder';
import * as rateOrderSchema from '../schemaValidator/rateOrder';
...
describe('Sync Status Order Booking', () => {
  test('Can sync status booking service order successfully using authorized token - positive test cases', async () => {
    ...
    bodyRateOrder.default.orderNo = await orderGet.orderNo;
    const responseRateOrder = await rateOrder(token, bodyRateOrder.default);
    expect(responseRateOrder.status).toEqual(200);
    expect(responseRateOrder.body).toMatchSchema(rateOrderSchema.default);
    expect(responseRateOrder.body).toMatchSnapshot();
  });
});
